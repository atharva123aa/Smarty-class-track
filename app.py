...
def save_question(q, opts, ans):
    with open(QUIZ_file, "a") as f:
        f.write(q + "|" + opts[0] + "|" + opts[1] + "|" + opts[2] + "|" + opts[3] + "|" + ans + "\n")

def get_questions():
    if not m.path.exists(QUIZ_file):
        return []
...
def submit_quiz():
    ...
    for i, q in enumerate(qs):
        ...
        if chosen == correct:
            ...
            results.append((q[0], chosen, correct, "wrong"))
...
def scores():
    studs = get_students()
    today=date.today().strftime("%d/%m/%Y")
    return render_template("scores.html", students=studs,today=today)
